...
Voice agent with mid-conversation tool injection, using Deepgram STT and Cartesia TTS over WebSocket transport.

Demonstrates `agent:update` with `update_tools` to add tools to the agent during a live conversation. The agent starts with no tools, and after the second conversational turn, a `web_search` tool is injected along with context informing the agent of its new capability.

Demonstrates:
- `session.updateAgent()` with `type: 'update_tools'`
