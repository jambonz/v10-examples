...
Voice agent with Tavily web search via MCP server, using Deepgram STT and Cartesia TTS over WebSocket transport.

Demonstrates external MCP server usage with Tavily's remote MCP endpoint. The agent discovers search tools from the MCP server automatically -- no inline tool definitions or toolHook needed.
