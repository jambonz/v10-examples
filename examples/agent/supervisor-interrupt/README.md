# agent/supervisor-interrupt

Voice agent with urgent message injection, using Deepgram STT and Cartesia TTS over WebSocket transport.

Demonstrates `agent:update` with `generate_reply` and `interrupt: true` to force the agent to deliver an urgent message. After the third agent response, the app waits 2 seconds then sends a flash sale notification that interrupts the current conversation.

Demonstrates:
- `session.updateAgent()` with `type: 'generate_reply'` and `interrupt: true`
