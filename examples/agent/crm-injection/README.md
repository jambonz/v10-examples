...
Voice agent with live CRM context injection, using Deepgram STT and Cartesia TTS over WebSocket transport.

Demonstrates `agent:update` with `inject_context` to push customer information into the conversation mid-call. After the first conversational turn, the app waits 3 seconds (simulating a CRM lookup) then injects customer details including name, account tier, and purchase history.

Demonstrates:
- `session.updateAgent()` with `type: 'inject_context'`
