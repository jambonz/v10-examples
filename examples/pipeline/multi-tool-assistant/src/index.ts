...
const envVars = {
  ...
  SYSTEM_PROMPT: {
    ...
    default: [
      'You are a helpful voice AI assistant with access to several tools.',
      'The user is interacting with you via voice.',
      'You can search the web for current information, look up weather,',
      'find facts on Wikipedia, do math calculations, and tell the time in any timezone.',
      'Use the appropriate tool when the user asks about any of these topics.',
      ...
      'greet the user and briefly mention what you can help with.',
    ].join(' '),
  },
  TEMPERATURE_SCALE: {
    type: 'string' as const,
    description: 'Temperature unit for weather lookups',
...
svc.on('session:new', (session) => {
  ...
  const tempScale = (session.data.env_vars?.TEMPERATURE_SCALE
    || envVars.TEMPERATURE_SCALE.default) as 'celsius' | 'fahrenheit';
  ...
  const tools = [
    ...
    createWeather({ scale: tempScale }),
    createWikipedia(),
    createCalculator(),
    createDateTime(),
  ];

  /* wire up tool dispatch — one line replaces all the manual handler boilerplate */
...
logger.info({ port }, 'jambonz pipeline/multi-tool-assistant listening');
